...
coefficient_C=$(jq -r '.context.coefficient_C ' benchmark.json)
echo "coefficient_C: $coefficient_C"
...
for ((nof_benchmark = 0; nof_benchmark < nof_benchmarks; nof_benchmark++)); do
  ...
  QUERY="INSERT INTO msm_benchmark (
  team,
  project,
  test_timestamp,
  git_id,
...
"
  ...
  # Execute the psql command to insert the row into the add_benchmark table
